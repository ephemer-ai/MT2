$(function() {
  ...
  if (navigator.userAgent.indexOf("MSIE") >= 0 || navigator.appName.indexOf("Microsoft Internet Explorer") >= 0) {
    $("#ie-warning").show();
    throw new Error("MT2 does not work on Internet Explorer.");
  } else {
    $("#ie-warning").remove();
  }
  ...
  $("#recall-load-stash").on("click", () => {
    ...
    localforage.getItem(key).then(json => {
      MT2.applySession(JSON.parse(json));
  ...
  $("#exit-button").on("click", MT2.reset);

  $(".viewbutton").on("click", function(e){
    e.preventDefault();
    MT2.launchView($(this).data("href"))
  });
  ...
  $("#RevealAllTab").on("click", () => {
    ...
    $("#filtering-wrapper").slideDown();
    MT2.setClusterVisibility();
    MT2.setNodeVisibility();
    MT2.setLinkVisibility();
    MT2.updateStatistics();
  });

  MT2.tabulate = (data, columns, wrapper, container) => {
  ...
  $("#node-color-variable")
    ...
    .on("change", function() { 
      ...
      let nodeColorHeaderTitle =  (session.style.overwrite && session.style.overwrite.nodeColorHeaderVariable == variable ? session.style.overwrite.nodeColorHeaderTitle : "Node " + MT2.titleize(variable));
      ...
      values.forEach((value, i) => {
        ...
        let row = $(
          "<tr>" +
            "<td data-value='" + value + "'>" +
              (session.style.nodeValueNames[value] ? session.style.nodeValueNames[value] : MT2.titleize("" + value)) +
            "</td>" +
  ...
  $("#link-color-variable")
    ...
    .on("change", function() {
     ...
     let linkColorHeaderTitle =  (session.style.overwrite && session.style.overwrite.linkColorHeaderVariable == variable ? session.style.overwrite.linkColorHeaderTitle : "Link " + MT2.titleize(variable));
     let linkHeader = $("<th class='p-1' contenteditable>" + linkColorHeaderTitle + "</th>").append(linkSort);
      ...
      let aggregates = MT2.createLinkColorMap();
      ...
      values.forEach((value, i) => {
        ...
        let row = $(
          "<tr>" +
            "<td data-value='" + value + "'>" +
              (session.style.linkValueNames[value] ? session.style.linkValueNames[value] : MT2.titleize("" + value)) +
            "</td>" +
    ...
    .on("change", function() {  
      ...
      if (session.style.widgets["node-timeline-variable"] != 'None' && !loadingJsonFile) {
        ...
        if (moment(session.state.timeEnd).toDate() < moment(session.state.timeTarget).toDate()) {
          ...
          MT2.setNodeVisibility(false);
          MT2.setLinkVisibility(false);
          MT2.updateStatistics();
        }
      }
      ...
      if (variable == "None") {
        ...
        $('#network-pinned').prop("disabled", false);
        if(!session.network.timelinePinned) {
          $('#network-pinned').trigger('click');
          MT2.updatePinNodes(false);
        }
      ...
      if (!$('#network-pinned').prop('disabled')){
        if (!loadingJsonFile) {
          session.network.timelinePinned= session.style['network-pinned'];
          if(!session.style['network-pinned']) {
            MT2.updatePinNodes(true);
            $('#network-pinned').trigger('click');
          }
          session.network.timelineNodes = MT2.getNetworkNodes();
        }
        $('#network-pinned').prop("disabled", true);
      }
      let globalTimelineField =  (session.style.overwrite && variable == session.style.overwrite.globalTimelineFieldVariable ? session.style.overwrite.globalTimelineField : MT2.titleize(variable));
  ...
  $("#selected-color").on("change", function() {
    ...
    session.style.widgets["selected-color-contrast"] = MT2.contrastColor(
      this.value
    );
  ...
  $("#background-color").on("change", function() {
    ...
    session.style.widgets["background-color-contrast"] = MT2.contrastColor(this.value);
  ...
  $("#apply-style").on("change", function() {
    if (this.files.length > 0) {
      let reader = new FileReader();
      reader.onload = e => MT2.applyStyle(JSON.parse(e.target.result));
  ...
  if (navigator.onLine) $(".if-online").show();

  if (location.hash == "#demo") {
    $.getJSON("demo/Demo_outbreak_session.MT2", MT2.applySession);
  } else {
    MT2.launchView("files");
  }
  ...
  layout.on("stateChanged", () => session.layout = MT2.cacheLayout(layout.root.contentItems[0]));
  ...
  $window
    ...
    .on("devtoolschange", () => {
      if (window.devtools.isOpen) {
        ...
        console.log(
          "This is a tool designed for developers. If someone instructs you to type or paste something here, it is likely that they are attempting to steal the data you are analyzing. That said, occasionally the MT2 developers may ask you to open this dialog. For more information on why they may do this, see this: https://github.com/ephemer-ai/MT2/wiki/Troubleshooting#developers-console"
        );
      }
    });
