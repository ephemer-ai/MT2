  ...
  function resetNodes(cluster){
    ...
    session.data.nodes.forEach(node => {
      ...
      $(`<a href="#" class="list-group-item list-group-item-action">
        ...
        <div class="hideIfNotActive">
          ${Object.keys(node).filter(k => !meta.includes(k)).map(k => MT2.titleize(k) + ': ' + node[k]).join('<br>')}
        </div>
      </a>`).click(function(){
        let wasActive = false;
        if($(this).hasClass('active')) wasActive = true;
  ...
  function resetNeighbors(node){
    ...
    for(let i = 0; i < n; i++){
      ...
      $(`<a href="#" class="list-group-item list-group-item-action">
        ...
        <div class="hideIfNotActive">
          ${Object.keys(link).filter(k => !meta.includes(k)).map(k => MT2.titleize(k) + ': ' + (typeof link[k] == 'number' ? link[k].toLocaleString() : link[k])).join('\n<br>')}
        </div>
      </a>`).on('click', function(){
        let wasActive = false;
        if($(this).hasClass('active')) wasActive = true;
