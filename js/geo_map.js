...
// save existing session styles
  ...
  if (navigator.onLine) {
    $('#map').parent().find('.if-online').css('display', 'flex');
  }
  ...
  function matchCoordinates(callback, norefresh) {
    if (!norefresh) nodes = MT2.getVisibleNodes();
  ...
  function makeGeoJSON() {
    ...
    MT2.getVisibleLinks().forEach(function (d) {
      if (!d.visible) return;
      var source = nodes.find(node => node._id == d.source);
      var target = nodes.find(node => node._id == d.target);
      if (source && target) {
